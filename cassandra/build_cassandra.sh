...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Temurin11" && "$JAVA_PROVIDED" != "OpenJDK11" && "$JAVA_PROVIDED" != "OpenJDK17" && "$JAVA_PROVIDED" != "Temurin17" ]]; then
        err "$JAVA_PROVIDED is not supported, only {Temurin11, OpenJDK11, OpenJDK17, Temurin17} are supported"
        exit 1
    fi
...
function build_netty-tcnative() {
    cd "$CURDIR"
    wget -q https://raw.githubusercontent.com/sudip-ibm/test/refs/heads/main/cassandra/build_netty.sh
    if [[ "$JAVA_PROVIDED" == "Temurin11" ]]; then
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "ubuntu-24.10" || "$DISTRO" == "rhel-"* || "$DISTRO" == "sles-"* ]]; then
        ...
        wget -q https://raw.githubusercontent.com/sudip-ibm/test/refs/heads/main/cassandra/build_python3.sh
    ...
    # Apply patch
    curl -sSL https://raw.githubusercontent.com/sudip-ibm/test/refs/heads/main/cassandra/cassandra.patch | git apply
...
function printHelp() {
    echo
    echo "Usage: "
    echo " bash build_cassandra.sh  [-d debug] [-y install-without-confirmation] [-t install-with-tests] [-j java to use [Temurin11, OpenJDK11, OpenJDK17, Temurin17]]"
